...
//                        }
       ...
       if(((usweight.getText().isEmpty())==false) && ((usheight.getText().isEmpty()==false)) && ((gender.getText().isEmpty())==false) && ((usage.getText().isEmpty()==false)) && ((password.getText().isEmpty()==false)) && ((uname.getText().isEmpty()==false)) && ((emailacc.getText().isEmpty()==false)) && ((usernamea.getText().isEmpty()==false)) )
       {
           ...
           {
                 // myConn = DriverManager.getConnection("jdbc:mysql://localhost:330/CalorieCut", "root", "passwordguess22@");
                    myConn = DriverManager.getConnection("jdbc:mysql://localhost:3306/CalorieCut", "root", "root");

//                    myStmt = myConn.createStatement();
                    //myRs = myStmt.executeQuery("select * from caloriecut.user where User_username='" + userTextField.getText() + "' and User_password='" + pwBox.getText() + "'");
                   String sql="INSERT INTO caloriecut.user (User_name,User_username,User_gender,User_age,User_height,User_weight,User_email,User_password) VALUES('"+una+"','"+userna+"','"+gend+"','"+age+"','"+height+"','"+weigh+"','"+ema+"','"+pass+"')";
                    //myConn.ps("INSERT INTO caloriecut.user (User_name,User_username,User_gender,User_age,User_height,User_weight,User_email,User_password) VALUES('"+una+"','"+userna+"','"+gend+"','"+age+"','"+height+"','"+weigh+"','"+ema+"','"+pass+"'");
//           ps=myConn.prepareStatement(sql);
//           ps.execute();
//
         ps=myConn.prepareStatement("insert into caloriecut.user (User_name,User_username,User_gender,User_age,User_height,User_weight,User_email,User_password)" + "values (?,?,?,?,?,?,?,?)");
         ps.setString(1, una);
         ps.setString(2, userna);
         ps.setString(3, gend);
         ps.setString(4, age);
         ps.setString(5, height);
         ps.setString(6, weigh);
         ps.setString(7, ema);
         ps.setString(8, pass);
